...
public interface BoardRepository extends JpaRepository<Board, Long>, BoardSearch { // 다중검색용 BoardSearch 추가 p451
    // extends JpaRepository<엔티티크래스, pk타입>
    // JpaRepository는 jpa에서 미리 만들어 놓은 인터페이스로 CRUD와 페이징처리, 정렬 등이 존재한다. (CRUD 쿼리가 들어 있다.)
    
    // 테스트 코드에서 JapRepository 에 내장된 CRUD와 페이징과 정렬 기법을 테스트함
    ...
    // 단점, 실제로 사용하려면 상당히 길고 복잡한 메서드명이 됨으로 많이 사용되지는 않는다.
    Page<Board> findByTitleContainingOrderByBnoDesc(String keyword, Pageable pageable);
    // 이터페이스에 구현메서드이므로 실행문이 없다. -> ;
    // findby 찾아오겠다. select
    // Title 제목필드 Containing
    // OrderBy 정렬진행 BnoDesc 번호를 내림차순


    //  @Query 쿼리 메서드와 병합 -> JPQL 
    ...
    // 단점, 조인과 같은 복잡한 쿼리를 사욯하지 못 함.
    //      원하는 속성들만 추출해서 객체배열 처리하거나 DTO로 처리 안 됨.
    //      네이티브 쿼리 속성값을 true로 지정해서 특정 데이터베이스에서 동작하는 sql 사용안됨
